...
- [ ] T001 Install `@tanstack/react-query@^5` — run `npm install @tanstack/react-query` at project root; verify entry appears in `package.json` dependencies
- [ ] T002 [P] Create `lib/types.ts` with three exports: `Seniority` type (`'junior' | 'mid' | 'senior'`), `Developer` interface (`id: string`, `name: string`, `avatar: string`, `seniority: Seniority`, `cost: number`, `skills: string[]`), and `FilterState` interface (`name: string`, `seniorities: Seniority[]`) — file: `lib/types.ts`
...
- [ ] T003 Create `app/providers.tsx` as a `'use client'` component that instantiates `QueryClient` with `defaultOptions: { queries: { staleTime: 5 * 60 * 1000, retry: 2 } }` via `useState` (prevents client recreation on re-render) and wraps `children` with `<QueryClientProvider client={queryClient}>` — file: `app/providers.tsx`
- [ ] T004 Create `app/(private)/layout.tsx` as a Server Component that imports `Providers` from `app/providers.tsx` and wraps `{children}` with `<Providers>`, providing the QueryClient scope to all private routes — file: `app/(private)/layout.tsx`
...
- [ ] T015 [P] Responsive grid audit — verify `CatalogueGrid.tsx` grid breakpoints (`grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4`) render correctly at mobile (375px), tablet (768px), and desktop (1280px); verify `DeveloperCard.tsx` has consistent height and no overflow at all breakpoints — file: `app/(private)/dashboard/_components/CatalogueGrid.tsx`
- [ ] T016 [P] Accessibility pass — add `aria-label="Search developers"` to the name `<input>` in `FilterBar.tsx`; add `aria-pressed={isActive}` to each seniority toggle button; confirm skeleton cards have `aria-busy` or are hidden from screen readers with `aria-hidden="true"` — file: `app/(private)/dashboard/_components/FilterBar.tsx`
- [ ] T017 Run full quickstart.md manual validation — execute all checklists: US1 (browse + skeleton + error), US2 (name filter), US3 (seniority multi-select + combined), US4 (error + retry), Visual Quality, Performance — file: `specs/002-developer-catalogue/quickstart.md`
